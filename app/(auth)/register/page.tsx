...
import { Mail, Lock, UserPlus, Loader2, AlertCircle, User } from "lucide-react";
...
export default function RegisterPage() {
  // Toda a lógica de estado, Zod e submissão resumida em uma linha:
      ...
      <Card className="w-full max-w-md border-slate-200/80 dark:border-slate-800/80 shadow-xl bg-white/80 dark:bg-slate-900/80 backdrop-blur-md transition-all duration-300 hover:shadow-2xl z-10">
        <CardHeader className="space-y-1 text-center pt-8">
          ...
          <CardDescription className="text-slate-500 dark:text-slate-400 text-sm">
            Cadastre-se para acessar o sistema da barbearia
          </CardDescription>
        </CardHeader>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                <User className="absolute left-3 top-1/2 -translate-y-1/2 h-4 w-4 text-slate-400 transition-colors group-focus-within:text-blue-500" />
                <Input
                  id="name"
                  type="text"
                  placeholder="Seu nome e sobrenome"
                  className={`pl-10 h-11 rounded-xl transition-all ${
                    errors.name ? "border-red-500 focus-visible:ring-red-500 bg-red-50/10" : "focus-visible:ring-blue-500 hover:border-slate-300 dark:hover:border-slate-700"
                  }`}
                  {...register("name")}
                />
              </div>
            ...
            <div className="space-y-1.5">
              ...
              <div className="relative group">
                ...
                <Input
                  ...
                  placeholder="••••••••"
                  className={`pl-10 h-11 rounded-xl transition-all ${
                    errors.confirmPassword ? "border-red-500 focus-visible:ring-red-500 bg-red-50/10" : "focus-visible:ring-blue-500 hover:border-slate-300 dark:hover:border-slate-700"
                  }`}
            ...
            <Button type="submit" className="w-full h-11 rounded-xl font-medium transition-all active:scale-[0.98] shadow-md bg-blue-600 hover:bg-blue-700 text-white cursor-pointer" disabled={isSubmitting}>
              {isSubmitting ? (
                <span className="flex items-center justify-center gap-2">
                  <Loader2 className="h-4 w-4 animate-spin" /> Criando sua conta...
                </span>
              ) : "Criar conta"}
            </Button>
            ...
            <div className="text-center text-sm text-slate-500">
              Já possui uma conta?{" "}
              <Link href="/login" className="text-blue-600 hover:text-blue-500 font-semibold underline-offset-4 hover:underline">
                Fazer login
              </Link>
            </div>
          </form>
